...
public class AwaitPayPresenter implements TabFragmentContract.Presenter {

    private Context mContext;
    private TabFragmentContract.UI mUI;
    private AllFragmentAdapter allFragmentAdapter;

    public AwaitPayPresenter(Context mContext, TabFragmentContract.UI mUI) {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, "取消失败,请稍后重试!", Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                    mUI.updateView();
                } catch (Exception e) {
    ...
    public void chaKanWuLiu(int index) {
        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("查看物流**%d", index), Toast.LENGTH_SHORT).show();
    }
    ...
    public void queRenShouHuo(int index) {
        ...
        request(RetrofitApi.createApi(Api.class).confirmOrders(mHashMap), new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, String.format("确认收货失败--%s", data), Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                } catch (Exception e) {
                }
            ...
            @Override
            public void onFail(Throwable e) {
                Toast.makeText(mContext, String.format("确认收货失败--%s", e.getMessage()), Toast.LENGTH_SHORT).show();
                mUI.hideLoad();
            }
        });

//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("确认收货**%d", index), Toast.LENGTH_SHORT).show();
    }
    ...
    public void shaiDanPingJia(int index) {
        ...
        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("晒单评价**%d", index), Toast.LENGTH_SHORT).show();
    }
    ...
    public void shanChuDingDan(int index) {
        ...
        request(RetrofitApi.createApi(Api.class).deleteOrders(mHashMap), new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, String.format("删除订单失败--%s", data), Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                } catch (Exception e) {
                }
            ...
            @Override
            public void onFail(Throwable e) {
                Toast.makeText(mContext, String.format("删除订单失败--%s", e.getMessage()), Toast.LENGTH_SHORT).show();
                mUI.hideLoad();
            }
        });

//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("删除订单**%d", index), Toast.LENGTH_SHORT).show();
    }
    ...
    public void shenQingShouHou(int index) {
        ...
        intent.putExtra("source_id",MineOrderActivity.FragmentType.AWAIT_ZHIFU);
        mContext.startActivity(intent);
    ...
    public void chaKanXiangQing(int index, int childIndex) {
        ...
        if (list.get(index).getOrderProductType() == 0) {
            OrderDetailsActivity.start(mContext, list.get(index).getOrders_id(), MineOrderActivity.FragmentType.AWAIT_ZHIFU,
                    list.get(index).getOrderProductType());
        } else {
            VirtualOrdersActivity.start(mContext, list.get(index).getOrders_id(),MineOrderActivity.FragmentType.AWAIT_ZHIFU);
        }
