...
import com.yunqin.bearmall.ui.fragment.presenter.AwaitShipmentsPresenter;
 ...
 * 代发货界面
 */
public class AwaitShipmentsFragment extends LazyFragment implements TabFragmentContract.UI, AllFragmentAdapter.OnChildClickListener {
    ...
    public void init() {
        ...
        isViewCreated = true;
        presenter = new AwaitShipmentsPresenter(getActivity(), this);
    ...
    protected void lazyLoad() {
        ...
        if (isPrepared && isVisible && !isFirst) {
            Log.e("CHENGPAN", "刷新*待发货*" + "AwaitShipmentsFragment");
            presenter.shuaXin();
        }
    ...
    public void jiaZaiGengDuoFinish(boolean isMore) {
        twinklingRefreshLayout.setEnableLoadmore(isMore);
        twinklingRefreshLayout.finishLoadmore();
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.e("TAG", AwaitShipmentsFragment.class.getSimpleName() + " onDestroy");
    }
    ...
    public void onUpdate(OrderTypeUpdate update) {
        if (update.getId() == MineOrderActivity.FragmentType.AWAIT_FAHUO) {
            isUpdate = true;
        }
    }
