...
public class AllFragmentPresenter implements TabFragmentContract.Presenter {
    ...
    public AllFragmentPresenter(Context mContext, TabFragmentContract.UI mUI) {
...
//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("取消**%d", index), Toast.LENGTH_SHORT).show();
    ...
    public void chaKanXiangQing(int index, int childIndex) {
        ...
        if (list.get(index).getOrderProductType() == 0) {
            OrderDetailsActivity.start(mContext, list.get(index).getOrders_id(),
                    MineOrderActivity.FragmentType.ALL_ORDER,
    ...
    public void jiaZaiGengDuo() {
        ...
        RetrofitApi.request(mContext, RetrofitApi.createApi(Api.class).getOrdersList(mHashMap), new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) {
                ...
                mUI.onHasMore(isMore);
                mUI.jiaZaiGengDuoFinish(isMore);
            }
