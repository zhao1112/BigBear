...
public class AwaitReceivePresenter implements TabFragmentContract.Presenter {

    private Context mContext;
    private TabFragmentContract.UI mUI;
    private AllFragmentAdapter allFragmentAdapter;

    public AwaitReceivePresenter(Context mContext, TabFragmentContract.UI mUI) {
...
//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("取消**%d", index), Toast.LENGTH_SHORT).show();
    ...
    public void queRenShouHuo(int index) {
        ...
        request(RetrofitApi.createApi(Api.class).confirmOrders(mHashMap), new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, String.format("确认收货失败,请稍后重试!", data), Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                    mUI.updateView();
                } catch (Exception e) {
            ...
            @Override
            public void onFail(Throwable e) {
                Toast.makeText(mContext, String.format("确认收货失败,请稍后重试!", e.getMessage()), Toast.LENGTH_SHORT).show();
                mUI.hideLoad();
            }
        });
...
//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("确认收货**%d", index), Toast.LENGTH_SHORT).show();
    ...
    public void shanChuDingDan(int index) {
        ...
        request(RetrofitApi.createApi(Api.class).deleteOrders(mHashMap), new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, String.format("删除订单失败,请稍后重试!", data), Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                } catch (Exception e) {
                }
            ...
            @Override
            public void onFail(Throwable e) {
                Toast.makeText(mContext, String.format("删除订单失败,请稍后重试!", e.getMessage()), Toast.LENGTH_SHORT).show();
                mUI.hideLoad();
            }
        });
...
//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("删除订单**%d", index), Toast.LENGTH_SHORT).show();
    ...
    public void shenQingShouHou(int index) {
        ...
        intent.putExtra("source_id",MineOrderActivity.FragmentType.AWAIT_SHOUHUO);
        mContext.startActivity(intent);
    }
    ...
    public void chaKanXiangQing(int index, int childIndex) {
        ...
        if (list.get(index).getOrderProductType() == 0) {
            OrderDetailsActivity.start(mContext, list.get(index).getOrders_id(), MineOrderActivity.FragmentType.AWAIT_SHOUHUO,
                    list.get(index).getOrderProductType());
        } else {
            VirtualOrdersActivity.start(mContext, list.get(index).getOrders_id(), MineOrderActivity.FragmentType.AWAIT_SHOUHUO);
        }
