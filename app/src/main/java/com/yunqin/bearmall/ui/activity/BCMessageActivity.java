...
import com.newversions.BCMAdapter;
...
public class BCMessageActivity extends BaseActivity implements View.OnClickListener {
    ...
    private BCMAdapter moneyRewardAdapter;
    ...
    private void initData() {
        ...
        getHttpData(mHashMap, new RetrofitApi.IResponseListener() {
            ...
            public void onSuccess(String data) throws JSONException {
                BCMessageData moneyRewardBean = new Gson().fromJson(data, BCMessageData.class);
                if (moneyRewardBean.getData() != null && moneyRewardBean.getData().getList() != null) {
                    ...
                    if (moneyRewardBean.getData().getList().size() > 0) {
                        mTwinklingRefreshLayout.setVisibility(View.VISIBLE);
                        moneyRewardAdapter = new BCMAdapter(BCMessageActivity.this, moneyRewardBean.getData().getList());
    ...
    private void changeView(boolean hasMore) {
        hasNext = hasMore;
        if (hasNext) {
            mTwinklingRefreshLayout.setBottomView(new RefreshBottomView(BCMessageActivity.this));
        } else {
            mTwinklingRefreshLayout.setBottomView(new RefreshFooterView(BCMessageActivity.this));
        }
    }
