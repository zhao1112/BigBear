...
public class AwaitShipmentsPresenter implements TabFragmentContract.Presenter {

    private Context mContext;
    private TabFragmentContract.UI mUI;
    private AllFragmentAdapter allFragmentAdapter;

    public AwaitShipmentsPresenter(Context mContext, TabFragmentContract.UI mUI) {
            ...
            public void onSuccess(String data) {
                try {
                    ...
                    if (code == 1) {
                        ...
                        Toast.makeText(mContext, String.format("取消失败", index), Toast.LENGTH_SHORT).show();
                    }
                    mUI.hideLoad();
                } catch (Exception e) {
                }
...
//        Toast.makeText(BearMallAplication.getInstance().getApplicationContext(), String.format("确认收货**%d", index), Toast.LENGTH_SHORT).show();
    ...
    public void shenQingShouHou(int index) {
        ...
        intent.putExtra("source_id",MineOrderActivity.FragmentType.AWAIT_FAHUO);
        mContext.startActivity(intent);
    }
    ...
    public void chaKanXiangQing(int index, int childIndex) {
        ...
        if (list.get(index).getOrderProductType() == 0) {
            OrderDetailsActivity.start(mContext, list.get(index).getOrders_id(), MineOrderActivity.FragmentType.AWAIT_FAHUO,
                    list.get(index).getOrderProductType());
        } else {
            VirtualOrdersActivity.start(mContext, list.get(index).getOrders_id(), MineOrderActivity.FragmentType.AWAIT_FAHUO);
        }
