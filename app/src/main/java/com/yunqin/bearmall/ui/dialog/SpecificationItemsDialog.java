...
import static com.yunqin.bearmall.ui.fragment.TrolleyFragment.ORDER_BRAND;
import static com.yunqin.bearmall.ui.fragment.TrolleyFragment.ORDER_ITEM;
import static com.yunqin.bearmall.ui.fragment.TrolleyFragment.ORDER_TYPE;

public class SpecificationItemsDialog extends BaseDialog implements View.OnClickListener {
    ...
    private int count = 1;
    private int type;
    private ProductDetail.Product mProduct;
...
//    private String mProductName;
     ...
     */
    public SpecificationItemsDialog(@NonNull Context context, ProductActivityPresenter mPresenter, ProductDetail.Product product, ProductDetail.Store store, long product_id, long sku_id, List<ProductDetail.SkuList> skuList) {
...
//        this.mProductImage = productImage;
        ...
        try {
            String jsonCache = FileUtils.readFile2String(mContext.getCacheDir().getAbsolutePath() + File.separator + "productInfo.txt", "UTF-8");
            Log.e("jsonCache", jsonCache);
            ProductDetail productDetail = new Gson().fromJson(jsonCache, ProductDetail.class);
            jsonSkuList = productDetail.getData().getSkuList();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        size = mProduct.getSpecificationItems().size();//规格分类个数
        skuSize = skuList.size();//规格分类个数

        //分类名称
        initspecificationItemsView(context, mProduct.getSpecificationItems());//构建规格名称textview
        ...
        if (mProduct.getProductImages().size() > 0) {
            Glide.with(mContext).setDefaultRequestOptions(BearMallAplication.getOptions(R.drawable.default_product_small)).load(mProduct.getProductImages().get(0).getThumbnail()).into(productImg);//显示缩略图
        }
        //是否用显示两个价格
        if (type == ProductDetailActivity.ADD_CART) {//加入购物车，需要显示法币和法币+bt两种价格
            productPriceTextView.setVisibility(View.GONE);
            productBTPriceTextView.setVisibility(View.VISIBLE);
        } else if (type == ProductDetailActivity.BUY_PRICE) {
            productPriceTextView.setVisibility(View.VISIBLE);
            productBTPriceTextView.setVisibility(View.GONE);
        } else if (type == ProductDetailActivity.BUY_BT_PRICE) {
            productPriceTextView.setVisibility(View.GONE);
            productBTPriceTextView.setVisibility(View.VISIBLE);
        }
...
//                        tv_minus.setBackgroundResource(R.drawable.specifition_item_minus_abled);
    ...
    private void priceAndspecificationItems() {
        ...
        BigDecimal mCountBigDecimal = new BigDecimal(String.valueOf(count));
        BigDecimal mPriceBigDecimal = new BigDecimal(mSkuListObject.getPrice());


        price = mPriceBigDecimal.multiply(mCountBigDecimal).floatValue();
