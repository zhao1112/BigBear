 ...
 * @author Master
 */
public class TopBanner2 extends RelativeLayout {
    ...
    //网络图片资源
    private List<HomeAd.DataBean.AdBean> mImageUrls;
    //是否只有一张图片
    ...
    public TopBanner2(Context context) {
        this(context, null);
    }

    public TopBanner2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TopBanner2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setImagesUrl(List<HomeAd.DataBean.AdBean> lists) {
        this.mImageUrls = lists;
        if (lists.size() <= 1) {
            mIsOneImg = true;
        }
        // 初始化ViewPager
    ...
    private class BannerPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(@NonNull ViewGroup container, final int position) {
            ...
            Glide.with(getContext()).setDefaultRequestOptions(BearMallAplication.getOptions(R.drawable.default_banner)).load(mImageUrls.get(toRealPosition(position)).getImg()).into(imageView);
